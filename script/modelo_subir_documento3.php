...
if($extension == 'pdf'){
    unlink($location.'rut.jpg');
    unlink($location.'rut.pdf');
    move_uploaded_file ($_FILES['file']['tmp_name'],$location.'rut.pdf');
}else{
    ...
    if($ancho>$alto){
        ...
        unlink($location.'rut.jpg');
        unlink($location.'rut.pdf');
        imagejpeg($imagen_optimizada, $location.'rut'.'.jpg');
    }else if($ancho<$alto){
...
$sql3 = "DELETE FROM modelos_documentos WHERE id_documentos = 4 and id_modelos = ".$modelo_id;
...
$sql2 = "INSERT INTO modelos_documentos (id_documentos,id_modelos,tipo,fecha_inicio) VALUES (4,'$modelo_id','$extension','$fecha_inicio')";
