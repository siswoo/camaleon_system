...
if($extension == 'pdf'){
    unlink($location.'pasaporte.jpg');
    unlink($location.'pasaporte.pdf');
    move_uploaded_file ($_FILES['file']['tmp_name'],$location.'pasaporte.pdf');
}else{
    ...
    if($ancho>$alto){
        ...
        unlink($location.'pasaporte.jpg');
        unlink($location.'pasaporte.pdf');
        imagejpeg($imagen_optimizada, $location.'pasaporte'.'.jpg');
    }else if($ancho<$alto){
...
$sql3 = "DELETE FROM modelos_documentos WHERE id_documentos = 3 and id_modelos = ".$modelo_id;
...
$sql2 = "INSERT INTO modelos_documentos (id_documentos,id_modelos,tipo,fecha_inicio) VALUES (3,'$modelo_id','$extension','$fecha_inicio')";
