...
for($i=2;$i<=$limite;$i++){
    if($worksheet->getCell('A'.$i)!=''){
        ...
        $coopserpak = $worksheet->getCell('L'.$i)->getvalue();
        ...
        if($contador1>=1){
            ...
            if($coopserpak!=''){
                ...
                $consulta4 = mysqli_query($conexion,$sql4);
                $sql5 = "INSERT INTO coopserpak (id_modelo,concepto,monto,fecha_desde,fecha_hasta,responsable,fecha_inicio) VALUES ($id_modelo,'Coopserpak','$coopserpak','$fecha_desde','$fecha_hasta','$responsable','$fecha_inicio')";
                $consulta5 = mysqli_query($conexion,$sql5);
            }
            if($multas!=''){
