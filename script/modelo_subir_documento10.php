...
if($extension == 'pdf'){
    unlink($location.'antecedentes_penales.jpg');
    unlink($location.'antecedentes_penales.pdf');
    move_uploaded_file ($_FILES['file']['tmp_name'],$location.'antecedentes_penales.pdf');
}else{
    ...
    if($ancho>$alto){
        ...
        unlink($location.'antecedentes_penales.jpg');
        unlink($location.'antecedentes_penales.pdf');
        imagejpeg($imagen_optimizada, $location.'antecedentes_penales'.'.jpg');
    }else if($ancho<$alto){
...
$sql3 = "DELETE FROM modelos_documentos WHERE id_documentos = 11 and id_modelos = ".$modelo_id;
...
$sql2 = "INSERT INTO modelos_documentos (id_documentos,id_modelos,tipo,fecha_inicio) VALUES (11,'$modelo_id','$extension','$fecha_inicio')";
