...
if($extension == 'pdf'){
    unlink($location.'foto_cedula_parte_respaldo.jpg');
    unlink($location.'foto_cedula_parte_respaldo.pdf');
    move_uploaded_file ($_FILES['file']['tmp_name'],$location.'foto_cedula_parte_respaldo.pdf');
}else{
    ...
    if($ancho>$alto){
        ...
        unlink($location.'foto_cedula_parte_respaldo.jpg');
        unlink($location.'foto_cedula_parte_respaldo.pdf');
        imagejpeg($imagen_optimizada, $location.'foto_cedula_parte_respaldo'.'.jpg');
    }else if($ancho<$alto){
...
$sql3 = "DELETE FROM modelos_documentos WHERE id_documentos = 10 and id_modelos = ".$modelo_id;
...
$sql2 = "INSERT INTO modelos_documentos (id_documentos,id_modelos,tipo,fecha_inicio) VALUES (10,'$modelo_id','$extension','$fecha_inicio')";
