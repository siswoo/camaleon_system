...
if($_SESSION["rol"]!=1){
	$sql1 = "SELECT * FROM presabana_inactivos WHERE sede = ".$_SESSION['sede']." and inicio BETWEEN '".$inicio."' AND '".$fin."' and fin BETWEEN '".$inicio."' AND '".$fin."' and total_dolares >=1";
	//$sql1 = "SELECT * FROM modelos WHERE sede = ".$_SESSION['sede'];
}else{
	$sql1 = "SELECT * FROM presabana_inactivos WHERE inicio BETWEEN '".$inicio."' AND '".$fin."' and fin BETWEEN '".$inicio."' AND '".$fin."' and total_dolares >=1";
}
		...
		$turno = $row2['turno'];
			...
			if($turno!='Satelite'){
				while($row8 = mysqli_fetch_array($consulta8)) {
					...
					$pre_bonos = $pre_bonos+$bonos_horas_valor;
				}
			}
			...
			if($pre_bonos>=1){
				$monto_separacion2 = $monto_separacion2+$pre_bonos;	
			}
