...
if($extension == 'pdf'){
    unlink($location.'certificacion_bancaria.jpg');
    unlink($location.'certificacion_bancaria.pdf');
    move_uploaded_file ($_FILES['file']['tmp_name'],$location.'certificacion_bancaria.pdf');
}else{
    ...
    if($ancho>$alto){
        ...
        unlink($location.'certificacion_bancaria.jpg');
        unlink($location.'certificacion_bancaria.pdf');
        imagejpeg($imagen_optimizada, $location.'certificacion_bancaria'.'.jpg');
    }else if($ancho<$alto){
...
$sql3 = "DELETE FROM modelos_documentos WHERE id_documentos = 5 and id_modelos = ".$modelo_id;
...
$sql2 = "INSERT INTO modelos_documentos (id_documentos,id_modelos,tipo,fecha_inicio) VALUES (5,'$modelo_id','$extension','$fecha_inicio')";
