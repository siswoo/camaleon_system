...
if($extension == 'pdf'){
    unlink($location.'examenes_ocupacionales.jpg');
    unlink($location.'examenes_ocupacionales.pdf');
    move_uploaded_file ($_FILES['file']['tmp_name'],$location.'examenes_ocupacionales.pdf');
}else{
    ...
    if($ancho>$alto){
        ...
        unlink($location.'examenes_ocupacionales.jpg');
        unlink($location.'examenes_ocupacionales.pdf');
        imagejpeg($imagen_optimizada, $location.'examenes_ocupacionales'.'.jpg');
    }else if($ancho<$alto){
...
$sql3 = "DELETE FROM modelos_documentos WHERE id_documentos = 15 and id_modelos = ".$modelo_id;
...
$sql2 = "INSERT INTO modelos_documentos (id_documentos,id_modelos,tipo,fecha_inicio) VALUES (15,'$modelo_id','$extension','$fecha_inicio')";
