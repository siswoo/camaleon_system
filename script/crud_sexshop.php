...
if($condicion=='table2'){
	...
	if($filtrado!=''){
		$filtrado = ' and (model.documento_numero LIKE "%'.$filtrado.'%" or model.nombre1 LIKE "%'.$filtrado.'%" or model.nombre2 LIKE "%'.$filtrado.'%" or model.apellido1 LIKE "%'.$filtrado.'%" or model.apellido2 LIKE "%'.$filtrado.'%" or sexs.fecha_desde LIKE "%'.$filtrado.'%" or sexs.fecha_inicio LIKE "%'.$filtrado.'%")';
	}
	...
	$sql1 = "SELECT model.id as mod_id, model.documento_numero as mod_documento_numero, model.documento_tipo as mod_documento_tipo, model.nombre1 as mod_nombre1, model.nombre2 as mod_nombre2, model.apellido1 as mod_apellido1, model.apellido2 as mod_apellido2, model.sede as mod_sede, model.apellido1 as mod_apellido1, model.estatus as mod_estatus, sed.nombre as sed_nombre, sexs.id as sexs_id, sexs.monto as sexs_monto, sexs.fecha_desde as sexs_fecha_desde, sexs.fecha_inicio as sexs_fecha_inicio FROM modelos model 
	INNER JOIN sedes sed 
	ON model.sede = sed.id 
	INNER JOIN sexshop sexs 
	ON model.id = sexs.id_modelo 
	WHERE model.id != 0 ".$filtrado." ".$sede." and sexs.responsable = ".$responsable;
	...
	INNER JOIN sexshop sexs 
	ON model.id = sexs.id_modelo 
	WHERE model.id != 0 ".$filtrado." ".$sede." and sexs.responsable = ".$responsable." ORDER BY sexs.id DESC LIMIT ".$limit." OFFSET ".$offset."";
	...
	if($conteo1>=1){
		while($row2 = mysqli_fetch_array($proceso2)) {
			...
			$mod_estatus = $row2["mod_estatus"];
			$sed_nombre = $row2["sed_nombre"];
			$sexs_monto = $row2["sexs_monto"];
			$sexs_fecha_desde = $row2["sexs_fecha_desde"];
			$sexs_fecha_inicio = $row2["sexs_fecha_inicio"];
			...
			$html .= '
		                <tr id="tr_'.$mod_id.'">
		                    ...
		                    <td style="text-align:center;">'.$mod_nombre1." ".$mod_nombre2." ".$mod_apellido1." ".$mod_apellido2.'</td>
		                    <td style="text-align:center;">'.$sed_nombre.'</td>
		                    <td style="text-align:center;">'.$sexs_monto.'</td>
		                    <td style="text-align:center;">'.$sexs_fecha_desde.'</td>
		                    <td style="text-align:center;">'.$sexs_fecha_inicio.'</td>
		                </tr>
			';
		}
	}else{
