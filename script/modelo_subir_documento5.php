...
if($extension == 'pdf'){
    unlink($location.'eps.jpg');
    unlink($location.'eps.pdf');
    move_uploaded_file ($_FILES['file']['tmp_name'],$location.'eps.pdf');
}else{
    ...
    if($ancho>$alto){
        ...
        unlink($location.'eps.jpg');
        unlink($location.'eps.pdf');
        imagejpeg($imagen_optimizada, $location.'eps'.'.jpg');
    }else if($ancho<$alto){
...
$sql3 = "DELETE FROM modelos_documentos WHERE id_documentos = 6 and id_modelos = ".$modelo_id;
...
$sql2 = "INSERT INTO modelos_documentos (id_documentos,id_modelos,tipo,fecha_inicio) VALUES (6,'$modelo_id','$extension','$fecha_inicio')";
