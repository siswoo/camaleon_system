	...
	<div class="seccion1" id="div_extras" style="display: none; padding: 5px 5px 5px 5px;">
		<div class="row">
			<div class="form-group col-12 text-center">
				...
				<button class="btn btn-primary ml-3" type="button" value="No" id="button_mostrar1" onclick="mostrarSeccion_final(this.id,value);">Descuento especial Masivo</button>
			</div>
		</div>
	</div>
...
<div id="div_mostrar1" style="display:none;">
	<div class="row">
		<div class="col-12 text-center" style="font-weight:bold; font-size: 20px;">Descuentos Masivos</div>
		<div class="col-3 form-group form-check">
			<label for="mostrar1_concepto" style="color:black; font-weight: bold;">Concepto</label>
			<input type="text" name="mostrar1_concepto" id="mostrar1_concepto" class="form-control">
		</div>
		<div class="col-3 form-group form-check">
			<label for="mostrar1_valor" style="color:black; font-weight: bold;">Valor</label>
			<input type="text" name="mostrar1_valor" id="mostrar1_valor" class="form-control">
		</div>
		<div class="col-3 form-group form-check">
			<select class="form-control" id="mostrar1_corte" name="mostrar1_corte">
				<option value="">Corte</option>
				<option value="1">1</option>
				<option value="2">2</option>
			</select>
			<select class="form-control" id="mostrar1_mes" name="mostrar1_mes">
		...
		<div class="col-3 form-group form-check">
			<input type="submit" class="btn btn-success" onclick="mostrar1_submit();">
		</div>
	</div>
</div>

<!-- NUEVO TOTALES PENDIENTES
<div class="container">
	<div class="row">
		<div class="col-12">
		...
		<div class="col-12 text-center mt-3 mb-3">
			<span style="font-weight: bold; font-size: 22px;">TOTALES</span>
		</div>
		<div class="col-6">
			<select class="form-control" id="totales_fecha" name="totales_fecha" required>
				<option value="">Seleccione</option>
				<?php
				$sql4 = "SELECT * FROM presabana GROUP BY inicio";
				$proceso4 = mysqli_query($conexion,$sql4);
				while ($row4=mysqli_fetch_array($proceso4)) {
					$presabana_id = $row4["id"];
					$presabana_inicio = $row4["inicio"];
					$presabana_fin = $row4["fin"];
					echo '<option value="'.$presabana_id.'">'.$presabana_inicio.' | '.$presabana_fin.'</option>';	
				}
				?>
			</select>
		</div>
		<div class="col-6 text-center">
			<button type="button" class="btn btn-info" value="0" onclick="totales1(value);">Consultar</button>
		</div>
	</div>
	<div class="row" style="margin-top: 4rem; margin-bottom: 3rem;" id="totales_resultado1"></div>
	<div class="col-12">
		<hr style="background-color: black;">
	</div>
</div>
...
<script type="text/javascript">
	...
	function mostrarSeccion_final(id,value){
		if(value=='Si'){
			$('#div_mostrar1').hide('slow');
			$('#button_mostrar1').val('No');
		}else{
			$('#div_mostrar1').show('slow');
			$('#button_mostrar1').val('Si');
		}
	}
	...
	function mostrar1_submit(){
		var mostrar1_concepto = $('#mostrar1_concepto').val();
		var mostrar1_valor = $('#mostrar1_valor').val();
		var mostrar1_corte = $('#mostrar1_corte').val();
		var mostrar1_mes = $('#mostrar1_mes').val();

		if(mostrar1_concepto=='' || mostrar1_valor=='' || mostrar1_corte=='' || mostrar1_mes==''){
			Swal.fire({
				title: 'Error',
		...
		$.ajax({
			type: 'POST',
			url: '../script/subir_descuentos_especiales1.php',
			dataType: "JSON",
			data: {
				"concepto": mostrar1_concepto,
				"valor": mostrar1_valor,
				"corte": mostrar1_corte,
				"mes": mostrar1_mes,
			},
	...
	function totales1(value){
		var totales_fecha = $('#totales_fecha').val();
		$.ajax({
			type: 'POST',
			url: '../script/crud_totales1.php',
			dataType: "JSON",
			data: {
				"value": value,
				"totales_fecha": totales_fecha,
				"condicion": "resultado1",
			},
			success: function(respuesta) {
				console.log(respuesta);
				//$('#totales_resultado1').html(respuesta["html"]);
			},
			error: function(respuesta) {
				console.log(respuesta["responseText"]);
			}
