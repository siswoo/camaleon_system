	    ...
	    <div class="row">
		    <div class="container_consulta1">
		    	<table id="example" class="table row-border hover table-bordered" style="font-size: 12px;">
			        ...
			        <tbody id="resultados">
			        	<?php
			        	$consulta2 = "SELECT * FROM paginas";
						$resultado2 = mysqli_query( $conexion, $consulta2 );
						while($row2 = mysqli_fetch_array($resultado2)) {
							$paginas_id 			= $row2['id'];
							$paginas_nombre 		= $row2['nombre'];
							$paginas_tasa			= $row2['tasa'];
							$paginas_url 			= $row2['url'];
							$paginas_moneda 		= $row2['moneda'];

							echo '
								<tr>
					                <td class="text-center">'.$paginas_nombre.'</td>
					                <td class="text-center">'.$paginas_tasa.'</td>
					                <td class="text-center"><a href="'.$paginas_url.'" target="blank_">'.$paginas_url.'</a></td>
					                <td class="text-center">'.$paginas_moneda.'</td>
					                <td class="text-center">
					            ';

					        echo '
								        <i class="fas fa-edit" style="color:#0095ff; cursor:pointer;" title="" value="'.$paginas_id.'" data-toggle="modal" data-target="#exampleModal" onclick="modal_edit('.$paginas_id.');"></i>
								        <i class="fas fa-trash-alt ml-3" style="color:red; cursor:pointer;" data-toggle="popover-hover" onclick="eliminar('.$paginas_id.');" value="'.$paginas_id.'"></i>
					        ';
	...
	<div class="modal fade" id="exampleModal3" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
		<div class="modal-dialog" role="document">
			<form action="#" method="POST" id="form_modal_nuevo" style="">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title" id="exampleModalLabel">Crear Página</h5>
						<button type="button" class="close" data-dismiss="modal" aria-label="Close">
							<span aria-hidden="true">&times;</span>
						</button>
					</div>
					<div class="modal-body">
					    <div class="row">
						    ...
						    <div class="col-6 form-group form-check">
							    <label for="tasa">Tasa <small style="color:red; font-weight: bold;">(*)</small></label>
							    <input type="text" name="tasa" id="tasa" value="" class="form-control" required>
						    </div>
						    <div class="col-6 form-group form-check">
							    <label for="url">Url <small style="color:red; font-weight: bold;">(*)</small></label>
							    <input type="text" name="url" id="url" value="" class="form-control" required>
						    </div>
						    <div class="col-6 form-group form-check">
							    <label for="moneda">Moneda <small style="color:red; font-weight: bold;">(*)</small></label>
...
<script type="text/javascript">
	...
	$("#form_modal_nuevo").on("submit", function(e){
	    ...
	    $.ajax({
			type: 'POST',
			url: '../script/guardar_paginas1.php',
