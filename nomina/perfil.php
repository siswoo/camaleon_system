...
<style type="text/css">
	...
	<div class="container">
		<ul class="nav nav-tabs mb-2">
			...
			<li class="nav-item">
				<a class="nav-link" href="#" id="Ddesprendibles" onclick="pestañas(this.id);" style="color:white; text-transform: uppercase;">Desprendibles</a>
			</li>
		</ul>
	<!--***********************************************************-->
	<!--**********************DATOS PERSONALES*********************-->
	...
	<form class="d-none" action="pdf1.php" method="POST" id="formulario11" target="_blank">
		<div class="row">
			...
			<div class="col-12 text-center form-group form-check">Pagos Accesibles
				<select class="form-control" id="desprendibles_corte" name="desprendibles_corte" required>
					<option value="">Seleccione</option>
					<?php
					$sql2 = "SELECT * FROM nomina_pagos_presabana WHERE id_nomina = $id_nomina_hidden GROUP BY fecha_desde";
					$proceso2 = mysqli_query($conexion,$sql2);
					while($row2=mysqli_fetch_array($proceso2)){
						$nomina_id = $row2["id"];
						$nomina_desde = $row2["fecha_desde"];
						$nomina_hasta = $row2["fecha_hasta"];
						echo '<option value="'.$nomina_id.'">'.$nomina_desde.' | '.$nomina_hasta.'</option>';
					}
					?>
				</select>
			</div>
	...
	function pestañas(variable){
		switch (variable) {
			case 'Dpersonales':
				...
				$('#Ddesprendibles').removeClass('active1');
			...
			case 'Ddesprendibles':
				...
				$('#Dpagos').removeClass('active1');
				$('#Dclave').removeClass('active1');
				$('#Ddesprendibles').addClass('active1');
