...
$sheet->setCellValue('A1', 'Sede Registrado');
$sheet->setCellValue('B1', 'Nombre Completo');
$sheet->setCellValue('C1', 'Estatus');
$fila = 2;

$sql1 = "SELECT * FROM pasantes ORDER BY sede";
...
while($row2 = mysqli_fetch_array($consulta)) {
	...
	$sheet->setCellValue('A'.$fila, $sede_nombre);
...
$writer->save('Todas las Pasantes '.$fecha_inicio.'.xlsx');
header("Location: Todas las Pasantes ".$fecha_inicio.".xlsx");
?>