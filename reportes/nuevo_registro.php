		...
		<form id="formulario2" method="POST">
			<div class="row">
			    <div class="col-4 text-center">
			    	...
			    	<input type="hidden" name="hidden_fecha" id="hidden_fecha" value="<?php echo $fecha_inicio; ?>">
			    </div>
			    <div class="col-4 text-center">
				...
				-->
				<div class="col-12 text-center">
			    	<input type="button" class="btn btn-info mt-3" onclick="generar1();" value="Generar Reporte">
			    </div>
			</div>
		</form>
			    <div class="col-12" style="padding-left: 1rem; padding-right: 1rem; display:none;" id="seccion2">
				    	...
				    	<tbody>
				    		<?php
					    		...
					    		while($row2 = mysqli_fetch_array($consulta2)) {
									...
									?>
										<td>
											<select class="form-control form-control-xs selectpicker" name="select_modelos_<?php echo $room_nombre; ?>" id="select_modelos_<?php echo $room_nombre; ?>" data-size="10" data-live-search="true" data-title="select_modelos" data-width="100%">
...
<script type="text/javascript">
	...
	function generar1(){
		var fecha = $('#hidden_fecha').val();
		var responsable = $('#responsable').val();
		var turno = $('#turno').val();
		//var fecha = $('#hidden_fecha').val();
		if(responsable=='' || turno==''){
			alert('debe señalar un monitor y un turno');
			$('#seccion2').hide();
		}else{
			$.ajax({
				type: 'POST',
				url: '../script/reporte_inicio_consulta1.php',
				data: $('#formulario2').serialize(),
				dataType: "JSON",
				...
				success: function(respuesta) {
					console.log(respuesta);
					$('#seccion2').show();
					for(var i=1;i<respuesta['contador1'];i++){
						$('#select_modelos_').val();
					}
				},
